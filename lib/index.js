...
var createHash = function (obj) { return murmurhash_js_1.murmur2(JSON.stringify(obj)).toString(); };
var defaultParams = {
    saveRoutes: true,
    fixtureBasePath: '.',
    hashFn: function (req) { return (__assign({}, req.body, req.cookies, req.params, req.query, { pathname: req.path })); },
    fixtureRoutes: false,
};
var getReqPath = function (req, params) {
    return path.resolve(params.fixtureBasePath, createHash(params.hashFn(req)) + ".json");
};
    ...
    return function (req, res, next) {
        ...
        if (params.fixtureRoutes) {
            readFileAsync(fixturePath)
                ...
                .catch(function (err) {
                console.warn("No fixture or wrongly formatted file for the path: " + fixturePath);
